...
# Class description
type TizenApp* = ref object of RootObj


# Event functions subclasses can override
method onCreate*(this: TizenApp) {. base .} = discard dlog_print(DLOG_DEBUG, "TizenApp", "App onCreate()")
method onPause*(this: TizenApp) {. base .} = discard dlog_print(DLOG_DEBUG, "TizenApp", "App onPause()")
method onResume*(this: TizenApp) {. base .} = discard dlog_print(DLOG_DEBUG, "TizenApp", "App onResume()")
method onTerminate*(this: TizenApp) {. base .} = discard dlog_print(DLOG_DEBUG, "TizenApp", "App onTerminate()")
method onLowMemory*(this: TizenApp) {. base .} = discard dlog_print(DLOG_DEBUG, "TizenApp", "App onLowMemory()")
method onAppControl*(this: TizenApp, event: app_control_h) {. base .} = discard dlog_print(DLOG_DEBUG, "TizenApp", "App onAppControl()")

# Execute the project
method run*(this: TizenApp) {. base, noReturn .} =
