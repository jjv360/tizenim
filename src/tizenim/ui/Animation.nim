...
type AnimationContext* = ref object of RootObj
    cinterop: CInterop

    # All animations to execute
    all*: seq[AnimationInfo]

    # Called when the first animation completes
    onCompleteHandlers: seq[OnAnimationComplete]
    ...
    ## Animation function
    tweenFunction: Elm_Transit_Tween_Mode
...
## Constructor
method init*(this: AnimationContext): AnimationContext {.base.} =

    # Set defaults
    this.cinterop = CInterop()
    this.duration = 0.3
    this.delay = 0
    this.tweenFunction = ELM_TRANSIT_TWEEN_MODE_SINUSOIDAL
    return this
...
method didComplete(this: AnimationContext) =
...
method addCompleteHandler*(this: AnimationContext, cb: OnAnimationComplete) {.private.} =
...
proc setupTransit(this: AnimationContext, anim: AnimationInfo, evasObject: Evas_Object, transit: Elm_Transit) =
...
proc finalizeTransit(this: AnimationContext, anim: AnimationInfo, evasObject: Evas_Object, transit: Elm_Transit) =
...
method execute*(this: AnimationContext) {.base.} =
...
method move*(this: AnimationContext, view: View, oldPos: Position, newPos: Position) =
